...
/**
 * Reads the raw V8 coverage snapshot saved by coverage-middleware.cjs,
 * converts it to Istanbul format via v8-to-istanbul, and prints a text
 * summary to the terminal.  An HTML report is also written to ./coverage/.
 *
 * Run by testem's on_exit hook after every test run.
 */
...
main().catch((err) => {
  ...
  process.exit(0); // Don't fail the test run due to a coverage error.
});
